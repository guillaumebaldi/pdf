 ...
 * <p/>
 * <ul>
 * <li>possible object sharing: {@link #valueOf(String)}, {@link #valueOf(CharSequence)}, {@link #valueOf(char)}
...
public class Strings {
    ...
    /**
     ...
     multivalent.std.adaptor.ManualPage
    ...
    static {
        for (int i = 0, imax = SHARED.length; i < imax; i++)
            SHARED[i] = String.valueOf((char) i).intern();
    }
    ...
    public static final Comparator DICTIONARY_ORDER = new Comparator() {
        public int compare(Object s1, Object s2) {
            return compareDictionary((String) s1, (String) s2, false);
        }
    };
     ...
     */
    public static final Comparator DICTIONARY_CASE_INSENSITIVE_ORDER = new Comparator() {
        public int compare(Object s1, Object s2) {
            return compareDictionary((String) s1, (String) s2, true);
        }
    };
    ...
    private static SoftReference sharedref_ = null;
    ...
    public static int compareDictionary(String s1, String s2, boolean caseinsensitive) {
        ...
        for (int i1 = 0, i2 = 0; i1 < len1 && i2 < len2; ) {
            ...
            if ('0' <= ch1 && ch1 <= '9' && '0' <= ch2 && ch2 <= '9') { // compare as individual digits -- numbers can be of any length
                ...
                int e1 = i1 + 1;
                while (e1 < len1 && '0' <= (ch1 = s1.charAt(e1)) && ch1 <= '9')
                    e1++;
                int e2 = i2 + 1;
                while (e2 < len2 && '0' <= (ch2 = s2.charAt(e2)) && ch2 <= '9')
                    e2++;
                ...
                int l1 = e1 - i1, l2 = e2 - i2;
                if (l1 != l2)
                    return l1 - l2;   // different number of digits => longer larger
...
//                assert i1==e1 && i2 == e2;
    ...
    public static String valueOf(String str) {
        ...
        if (str == null)
            ...
            s = "";
        else if (str.length() == 1 && str.charAt(0) < SHARED.length)
            s = SHARED[str.charAt(0)];
        else {
            Map smap = sharedref_ != null ? (Map) sharedref_.get() : null;
            if (smap == null) {
                smap = new HashMap(2000);
                sharedref_ = new SoftReference(smap);
            }
            String sstr = (String) smap.get(str);
            if (sstr != null)
                s = sstr;
            else {
                s = str;
     ...
     */
    public static String valueOf(CharSequence sb) {
        String s;
        if (sb == null)
            s = null;
        else if (sb.length() == 0)
            s = "";
        else if (sb.length() == 1 && sb.charAt(0) < SHARED.length)
            s = SHARED[sb.charAt(0)];
        else
            s = valueOf(sb.toString());   // vs .toString() -- space or speed?
        return s;
    }

    public static String valueOf(char ch) {
        return ch < SHARED.length ? SHARED[ch] : String.valueOf(ch);
    }
    ...
    public static String valueOf(byte[] b) {
        return StringBuffers.valueOf(b).toString();
    }
...
//        for (String s : strs) {
        for (int i = 0, max = strs.length; i < max; i++) {
            len = len + strs[i].length() + join.length();
        }
        StringBuilder sb = new StringBuilder(len - join.length());
        sb.append(strs[0]);
    ...
    public static String trimWhitespace(String txt) {
        return trimWhitespace(txt, 0, txt.length() - 1);
    }
...
*/
     ...
     */
    public static String casify(String words, String wordbreak, Map exceptions) { // like String.toUpperCase()
...
//	int start=0, end=-1;
        ...
        for (int start = 0, end = start, len = words.length(); end < len; end++, start = end) {
            ...
            for (end = start; end < len; end++)
                if (wordbreak.indexOf(ch = words.charAt(end)) != -1) break;
            if (end == start) {
                sb.append(ch);
                continue;
            }    // consecutive split chars
            String word = words.substring(start, end);

//		if (start>0) sb.append(' ');
//System.out.println(word+" => "+lcexceptions.get(word));
            boolean alluc = true;
            for (int i = 0, imax = word.length(); alluc && i < imax; i++)
                if (Character.isLowerCase(word.charAt(i))) alluc = false;
            if (!alluc)
                sb.append(word); // leave alone words with any lowercase
            else if (exceptions != null && (newword = (String) exceptions.get(word)) != null)
                sb.append(newword);
...
//		if (end==-1 || end==len) break; else start=end;
        ...
        for (int i = 0, imax = txt.length(); i < imax; i++) {
            ...
            if (ch >= 128) {
                ...
                for (int j = 0, jmax = txt.length(); j < jmax; j++) {
                    ...
                    if (ch < 128)
                        sb.append(ch);
                    else if (OFF_ACCENT <= ch && ch < end && u2a[ch - OFF_ACCENT] != null)
                        sb.append(u2a[ch - OFF_ACCENT]);
                    else if ('\ufb00' <= ch && ch <= '\ufb04')
                        sb.append(LIGATURE2ASCII[ch - 0xfb00]);
                    else if ('\u2018' <= ch && ch <= '\u201e')
                        sb.append(QUOTE2ASCII[ch - 0x2018]);
                    else if ('\u2013' <= ch && ch <= '\u2014')
                        sb.append("--");    // endash and emdash
                    //else skip char
                }
                txt = sb.toString();
                break;
    ...
    public static String fromPigLatin(String str) {
        ...
        while (pend > 0 && org.icepdf.core.pobjects.fonts.nfont.lang.Strings.PUNCT.indexOf(str.charAt(pend - 1)) != -1)
            pend--;
        int wend = pend;
//System.out.println(wend+"/"+str+"/"+(wend>2? Character.toLowerCase(str.charAt(wend-2)): ' ')+"/"+(wend>2? Character.toLowerCase(str.charAt(wend-1)): ' '));
        if (wend > 2 && Character.toLowerCase(str.charAt(wend - 2)) == 'a' && Character.toLowerCase(str.charAt(wend - 1)) == 'y') {
            ...
            StringBuilder sb = new StringBuilder(wend);
            // letters moved to end
            int hy = str.lastIndexOf('-');
            if (hy != -1 && hy < wend) {
                char c0 = str.charAt(hy + 1);
                if (hy + 1 < wend)
                    sb.append(Character.isUpperCase(str.charAt(0)) ? Character.toUpperCase(c0) : c0);
