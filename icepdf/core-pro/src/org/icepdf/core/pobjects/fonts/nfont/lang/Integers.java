...
public class Integers {
    //public static final Intger[] ARRAY0 = new Integer[0];
    ...
    static {
        ...
        for (int base = 1; base < 256; base *= 2) {
            for (int i = 0; i < base; i++)
                BIT_COUNT[base + i] = (byte) (BIT_COUNT[i] + 1);    // old count + high bit set
        }
    }
    ...
    public static int parseInt(int ch) {
        if (RADIX == null) {
            ...
            for (int i = 10, a = 'a', A = 'A'; a <= 'z'; i++, a++, A++)
                RADIX[a] = RADIX[A] = i;
        }
    ...
    public static int parseHex(int ch1, int ch2) {
        return (parseInt(ch1) << 4) | parseInt(ch2);
    }
    ...
    public static Integer getInteger(int val) {
        ...
        if (INTS_MIN <= val && val <= INTS_MAX) {
            ...
            into = new Integer(val);    // later maybe keep hash of last N
...
//if (INTS_MIN<=val && val<=INTS_MAX) ihist[val-INTS_MIN]++;
    ...
    public static String toRomanString(int val) {   // like toHexString
        ...
        if (val <= 0 || val >= 4000) /*throw NumberFormatException()*/
            return Integer.toString(val);

        if (val < ROMAN.length)
            return ROMAN[val];  // fast path for small numbers -- no object creation

        // i=1, v=5, x=10, l=50, c=100, d=500, m=1000, bar=multiply by 1000, no 0
        StringBuilder rn = new StringBuilder(10);
        while (val >= 1000) {
            rn.append('M');
            val -= 1000;
        }
...
//        assert ROMAN.length >= 50;
    ...
    public static String toString(int val) {
        ...
        if (STRS_MIN <= val && val <= STRS_MAX) {
            if (STRS == null)
                STRS = new String[(int) (STRS_MAX - STRS_MIN + 1)];
