 ...
 * <li>{@link #concat(Object[], Object[])} returns new array that is concatenation of passed arrays
...
//        assert a!=null && b != null;	// allow null for b?
     ...
     * If array is sorted, use {@link binarySearch(byte[], byte)} instead.
     */
    ...
    /**
     * Finds first index of <var>key<var> in <var>a</var> or -1 if not present.
     * To search for string of 8-bit characters, first convert string to <code>byte[]</code> with {@link org.icepdf.core.pobjects.fonts.nfont.lang.Strings#getBytes8(String)}.
     */
    public static int indexOf(byte[] a, byte[] key) {
        return indexOf(a, key, 0, a.length);
    }

    public static int indexOf(byte[] a, byte[] key, int start) {
        return indexOf(a, key, start, a.length);
    }
    ...
    public static int indexOf(byte[] a, byte[] key, int start, int end) {
        ...
        for (int i = start, imax = end - key.length; i < imax; i++) {
            if (key0 == a[i]) {
                boolean fmatch = true;
                for (int j = 0 + 1, jmax = key.length; j < jmax; j++)
                    if (key[j] != a[i + j]) {
                        fmatch = false;
                        break;
                    }
    ...
    public static boolean isSorted(Object[] oa) {
        if (oa != null) for (int i = 0, imax = oa.length - 1; i < imax; i++)
            if (((Comparable) oa[i]).compareTo(oa[i + 1]) > 0) return false;
        return true;
    }
    ...
    public static boolean isSorted(Object[] oa, Comparator comp) {
        if (oa != null) for (int i = 0, imax = oa.length - 1; i < imax; i++)
            if (comp.compare(oa[i], oa[i + 1]) > 0) return false;
        return true;
    }

    public static boolean isSorted(int[] ia) {
        if (ia != null) for (int i = 0, imax = ia.length - 1; i < imax; i++)
            if (ia[i] > ia[i + 1]) return false;
        return true;
    }
